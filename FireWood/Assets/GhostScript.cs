...
public class GhostScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector2.Distance(player.transform.position, transform.position) < player.CurrentRange || isLit
         || Vector2.Distance(forester.transform.position, transform.position) < forester.CurrentRange)
        {
            coroutine = StartCoroutine(Disappear());
        }
        else if (Vector2.Distance(player.transform.position, transform.position) > player.CurrentRange * 4 && !isLit 
